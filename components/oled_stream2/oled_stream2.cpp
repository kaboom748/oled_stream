...
static const char *const TAG = "oled_stream2";
...
<div class="label">Emulateur OLED - oled_stream2</div>
...
// ===================================================================
// Couche réseau — branche ESP8266 (WiFiServer, prouvé fonctionnel)
// ===================================================================
#ifdef USE_ESP8266

void OledStream2::setup() {
...
void OledStream2::send_html_(WiFiClient &client) {
...
// ===================================================================
// Couche réseau — branche ESP32 (esphome::socket)
// ===================================================================
...
void OledStream2::setup() {
  ...
  if (!this->server_socket_) {
    ESP_LOGE(TAG, "Échec création socket (errno=%d)", errno);
    return;
  }
  ...
  if (this->server_socket_->bind((struct sockaddr *) &server_addr, sl) != 0) {
    ESP_LOGE(TAG, "Échec bind() port %u (errno=%d)", this->port_, errno);
    return;
  }
  if (this->server_socket_->listen(4) != 0) {
    ESP_LOGE(TAG, "Échec listen() (errno=%d)", errno);
    return;
  }
...
void OledStream2::loop() {
  ...
  if (new_client) {
    ...
    if (request.find("GET /buffer") != std::string::npos) {
      if (this->stream_client_) this->stream_client_.reset();

      std::string headers;
      headers += "HTTP/1.1 200 OK\r\n";
      headers += "Content-Type: application/octet-stream\r\n";
      headers += "Access-Control-Allow-Origin: *\r\n";
      headers += "Access-Control-Expose-Headers: X-Display-Type, X-Buffer-Layout, X-Display-Width, X-Display-Height, X-Bits-Per-Pixel, X-Buffer-Size\r\n";
      headers += "X-Display-Type: " + this->display_type_ + "\r\n";
      headers += "X-Buffer-Layout: " + this->buffer_layout_ + "\r\n";
      headers += "X-Display-Width: " + std::to_string(this->width_) + "\r\n";
      headers += "X-Display-Height: " + std::to_string(this->height_) + "\r\n";
      headers += "X-Bits-Per-Pixel: " + std::to_string(this->bits_per_pixel_) + "\r\n";
      headers += "X-Buffer-Size: " + std::to_string(this->buffer_size_) + "\r\n";
      headers += "Connection: keep-alive\r\n\r\n";

      new_client->write(headers.c_str(), headers.size());
...
void OledStream2::send_html_(std::unique_ptr<socket::Socket> &client) {
  std::string headers = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n";
  client->write(headers.c_str(), headers.size());

  size_t html_len = strlen_P(EMULATOR_HTML_TEMPLATE);
  char chunk[257];
  for (size_t i = 0; i < html_len; i += 256) {
    size_t clen = std::min((size_t)256, html_len - i);
    memcpy_P(chunk, EMULATOR_HTML_TEMPLATE + i, clen);
    client->write(chunk, clen);
  }
}
