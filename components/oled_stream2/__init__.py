...
oled_stream2_ns = cg.esphome_ns.namespace("oled_stream2")
OledStream2 = oled_stream2_ns.class_("OledStream2", cg.Component)
...
CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(): cv.declare_id(OledStream2),
...
_DEFAULT_INFO = ("Inconnu", "mono_page", 1)
...
async def to_code(config):
    ...
    cg.add(var.set_display_dimensions(config[CONF_WIDTH], config[CONF_HEIGHT]))
